...
static bool dispatch_highpri(bool from_timer) {
  ...
  bpf_for_each(scx_dsq, p, SHARED_DSQ, 0) {
    ...
    if (tctx->highpri) {
      ...
      __COMPAT_scx_bpf_dsq_move_set_slice(BPF_FOR_EACH_ITER, slice_ns * 2);
      __COMPAT_scx_bpf_dsq_move_set_vtime(BPF_FOR_EACH_ITER, highpri_seq++);
      __COMPAT_scx_bpf_dsq_move_vtime(BPF_FOR_EACH_ITER, p, HIGHPRI_DSQ, 0);
    }
  }
  ...
  bpf_for_each(scx_dsq, p, HIGHPRI_DSQ, 0) {
    ...
    if (__COMPAT_scx_bpf_dsq_move(BPF_FOR_EACH_ITER, p, SCX_DSQ_LOCAL_ON | cpu,
                                  SCX_ENQ_PREEMPT)) {
      if (cpu == this_cpu) {
        dispatched = true;
...
void BPF_STRUCT_OPS(qmap_dump_cpu, struct scx_dump_ctx *dctx, s32 cpu,
  ...
  scx_bpf_dump(
      ...
      cpuc->dsp_idx, cpuc->dsp_cnt, cpuc->avg_weight, cpuc->cpuperf_target);
}
...
void BPF_STRUCT_OPS(qmap_dump_task, struct scx_dump_ctx *dctx,
  ...
  scx_bpf_dump("QMAP: force_local=%d core_sched_seq=%llu", taskc->force_local,
               taskc->core_sched_seq);
}

s32 BPF_STRUCT_OPS(qmap_cgroup_init, struct cgroup *cgrp,
                   struct scx_cgroup_init_args *args) {
  bpf_printk("CGRP INIT %llu weight=%u period=%lu quota=%ld burst=%lu",
             cgrp->kn->id, args->weight, args->bw_period_us, args->bw_quota_us,
             args->bw_burst_us);
  return 0;
}
...
SCX_OPS_DEFINE(sched_ops, .select_cpu = (void *)qmap_select_cpu,
               ...
               .init_task = (void *)qmap_init_task, .dump = (void *)qmap_dump,
               ...
               .init = (void *)qmap_init, .exit = (void *)qmap_exit,
               .timeout_ms = 5000U, .name = "qmap");
