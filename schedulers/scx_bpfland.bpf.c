...
volatile u64 nr_kthread_dispatches, nr_direct_dispatches, nr_shared_dispatches,
    ...
    nr_idle_pick_failures, nr_idle_primary_domain_misses, nr_idle_global_misses,
    nr_waker_cpu_biases, nr_keep_running_reuses, nr_keep_running_queue_empty,
 ...
 */
private
(BPFLAND) struct bpf_cpumask __kptr *primary_cpumask;
...
static s32 pick_idle_cpu(struct task_struct *p, s32 prev_cpu, s32 this_cpu,
  ...
  if (primary_all && is_wakeup(wake_flags) && this_cpu >= 0 &&
     ...
     * don't migrate.
     */
    if (cpus_share_cache(this_cpu, prev_cpu) && !is_smt_contended(prev_cpu) &&
        scx_bpf_test_and_clear_cpu_idle(prev_cpu))
      return prev_cpu;
 ...
 */
s32 BPF_STRUCT_OPS(bpfland_select_cpu, struct task_struct *p, s32 prev_cpu,
                   u64 wake_flags) {
  ...
  if (cpu >= 0) {
    ...
    if (tctx) {
      ...
      scx_bpf_dsq_insert_vtime(p, cpu_dsq(cpu), task_slice(p, cpu),
...
void BPF_STRUCT_OPS(bpfland_enqueue, struct task_struct *p, u64 enq_flags) {
  ...
  if (is_pcpu_task(p)) {
    if (local_pcpu)
      ...
      scx_bpf_dsq_insert_vtime(p, cpu_dsq(prev_cpu), task_slice(p, prev_cpu),
...
void BPF_STRUCT_OPS(bpfland_cpu_release, s32 cpu,
...
SCX_OPS_DEFINE(sched_ops, .select_cpu = (void *)bpfland_select_cpu,
