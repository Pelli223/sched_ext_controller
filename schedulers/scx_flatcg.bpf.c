...
static struct fcg_cgrp_ctx *find_ancestor_cgrp_ctx(struct cgroup *cgrp,
                                                   int level) {
  struct fcg_cgrp_ctx *cgc;
...
static void cgrp_refresh_hweight(struct cgroup *cgrp,
                                 struct fcg_cgrp_ctx *cgc) {
  int level;
  ...
  bpf_for(level, 0, cgrp->level + 1) {
    ...
    if (!level) {
      ...
      if (is_active) {
        ...
        cgc->hweight =
            div_round_up(pcgc->hweight * cgc->weight, pcgc->child_weight_sum);
      }
...
static void cgrp_cap_budget(struct cgv_node *cgv_node,
                            struct fcg_cgrp_ctx *cgc) {
   ...
   */
  max_budget = (cgrp_slice_ns * nr_cpus * cgc->hweight) / (2 * FCG_HWEIGHT_ONE);
...
s32 BPF_STRUCT_OPS(fcg_select_cpu, struct task_struct *p, s32 prev_cpu,
                   u64 wake_flags) {
...
int BPF_STRUCT_OPS_SLEEPABLE(fcg_cgroup_init, struct cgroup *cgrp,
  ...
  cgc =
      bpf_cgrp_storage_get(&cgrp_ctx, cgrp, 0, BPF_LOCAL_STORAGE_GET_F_CREATE);
  if (!cgc) {
    ret = -ENOMEM;
  ...
  ret = bpf_map_update_elem(&cgv_node_stash, &cgid, &empty_stash, BPF_NOEXIST);
  if (ret) {
    if (ret != -ENOMEM)
      scx_bpf_error("unexpected stash creation error (%d)", ret);
    goto err_destroy_dsq;
  }
...
void BPF_STRUCT_OPS(fcg_cgroup_move, struct task_struct *p, struct cgroup *from,
                    struct cgroup *to) {
...
void BPF_STRUCT_OPS(fcg_exit, struct scx_exit_info *ei) { UEI_RECORD(uei, ei); }

SCX_OPS_DEFINE(flatcg_ops, .select_cpu = (void *)fcg_select_cpu,
               .enqueue = (void *)fcg_enqueue, .dispatch = (void *)fcg_dispatch,
               .runnable = (void *)fcg_runnable, .running = (void *)fcg_running,
               ...
               .cgroup_move = (void *)fcg_cgroup_move, .init = (void *)fcg_init,
               .exit = (void *)fcg_exit, .flags = SCX_OPS_ENQ_EXITING,
               .name = "flatcg");
