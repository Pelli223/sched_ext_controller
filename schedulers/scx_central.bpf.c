...
s32 BPF_STRUCT_OPS(central_select_cpu, struct task_struct *p, s32 prev_cpu,
                   u64 wake_flags) {
  /*
...
static int central_timerfn(void *map, int *key, struct bpf_timer *timer) {
  ...
  if (timer_pinned && (curr_cpu != central_cpu)) {
    scx_bpf_error("Central timer ran on CPU %d, not central CPU %d", curr_cpu,
                  central_cpu);
    return 0;
  }
  ...
  bpf_for(i, 0, nr_cpu_ids) {
    ...
    if (started_at && *started_at && time_before(now, *started_at + slice_ns))
      continue;
...
SCX_OPS_DEFINE(sched_ops,
               ...
               .init = (void *)central_init, .exit = (void *)central_exit,
               .name = "central");
