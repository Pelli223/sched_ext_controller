...
int BPF_STRUCT_OPS(sched_enqueue, struct task_struct *p, u64 enq_flags) {
  // Calculate the time slice for the task based on the number of tasks in the
  // queue
...
int BPF_STRUCT_OPS(sched_dispatch, s32 cpu, struct task_struct *prev) {
  ...
  u32 weight_sum = 0;
  bpf_for_each(scx_dsq, p, SHARED_DSQ_ID, 0) { weight_sum += p->scx.weight; };

  s32 random = bpf_get_prandom_u32() % weight_sum;
  bpf_for_each(scx_dsq, p, SHARED_DSQ_ID, 0) {
    random = random - p->scx.weight;
    if (random <= 0 && bpf_cpumask_test_cpu(cpu, p->cpus_ptr) &&
        scx_bpf_dsq_move(BPF_FOR_EACH_ITER, p, SCX_DSQ_LOCAL_ON | cpu,
                         SCX_ENQ_PREEMPT)) {
      bpf_printk("Dispatched task %s to CPU %d, %d of %d", p->comm, cpu,
                 p->scx.weight, weight_sum);
      return 0;
    }
  };
  return 0;
